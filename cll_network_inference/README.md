...
- [Building Block cll_network_inference](#building-block-new_name)
...
```bash
pip install cll_network_inference
```

### Usage

The `cll_network_inference` package provides a clear interface that allows it to be used with multiple workflow managers (e.g. PyCOMPSs, NextFlow and Snakemake).

- Command line interface:

    Once installed the `cll_network_inference` package, it provides the `cll_network_inference`
    command, that can be used from the command line. For example:

    ```text
    $ cll_network_inference -h
    usage: cll_network_inference [-h] --model MODEL --result RESULT [-c CONFIG] [-d] [-l {debug,info,warning,error,critical}] [--tmpdir TMPDIR] [--processes PROCESSES] [--gpus GPUS] [--memory MEMORY]
                [--mount_points MOUNT_POINTS]

    cll_network_inference Building Block short description. Give more details about the Building Block.
    ...
    ```python
    from cll_network_inference import invoke
...
- Extension for PyCOMPSs:
    ...
    ```python
    from cll_network_inference import cll_network_inference_extended

    cll_network_inference_extended(*args, **kwargs)  # specific interface
    ```
...
```bash
pip uninstall cll_network_inference
```
...
- Provide a Python script `main.py` with the following structure and adapt to your needs (check all `ToDo` marked lines):
  ...
  from cll_network_inference.definitions import cll_network_inference_ASSETS_PATH  # binary could be in this folder
  from cll_network_inference.definitions import cll_network_inference_CONTAINER
  ...
  @binary(binary="cp")                                        # ToDo: Define the binary to be used (can be within cll_network_inference_ASSETS_PATH (e.g. my_binary.sh)).
  ...
  - A BB can be a single executable, but it can be a more complex code if the `cll_network_inference_extended` function is implemented and used with PyCOMPSs.
  ...
  parses the Yaml config file and invokes the `cll_network_inference` function
  with the appropriate parameters.

  - The BB `binary` must be defined with the `@task`, `@binary` and `@container` decorators (`cll_network_inference_task`). This function needs to declare the binary flags, and it is invoked from the `cll_network_inference` function.
