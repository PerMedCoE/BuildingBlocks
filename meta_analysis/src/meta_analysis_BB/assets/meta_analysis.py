...
def create_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Choose which results to analyse",
    )
    parser.add_argument(
        "-o",
        "--out_dir",
        type=str,
        default=None,
        help="Inform how many replicated where done",
    )
    parser.add_argument(
        "-p",
        "--model_prefix",
        type=str,
        default=None,
        help="Number of cores to use for analysing results",
    )
    parser.add_argument(
        "-k",
        "--ko_file",
        type=str,
        default=None,
        help="Default dir format for PhysiCell replicates",
    )
    parser.add_argument(
        "-r", "--repetitions", type=int, default=1, help="Number of replicates"
    )
    parser.add_argument("-v", "--verbose", type=bool, default=False, help="Verbosity")
    parser.add_argument(
        "-z", "--result", type=str, default=None, help="result directory"
    )
    return parser
...
def main():
    ...
    a = sio.loadmat(
        os.path.join(
            ...
            "%s_personalized_%sresults" % (args.model_prefix, genes[0]),
            "timeReplicate.mat",
        )
    )
    ...
    epi_traces = np.zeros(
        (len(genes) * len(patients) * args.repetitions, max_timestep - start_timestep)
    )
    cd8_traces = np.zeros(
        (len(genes) * len(patients) * args.repetitions, max_timestep - start_timestep)
    )
    ...
    for i_patient, patient in enumerate(patients):
        ...
        for i_gene, gene in enumerate(genes):
            gene_path = os.path.join(
                patient_data_path,
                "%s_personalized_%sresults" % (args.model_prefix, gene),
            )
            ...
            cellsb = b["tracedata"]
            epi_results[i_patient * len(genes) + i_gene, :] = cells[
                0, 16, start_timestep:max_timestep
            ]
            mac_results[i_patient * len(genes) + i_gene, :] = np.sum(
                cells[0, 1:5, start_timestep:max_timestep], axis=0
            )
            cd8_results[i_patient * len(genes) + i_gene, :] = cells[
                0, 0, start_timestep:max_timestep
            ]
            ind_start = (
                i_patient * len(genes) * args.repetitions + i_gene * args.repetitions
            )
            epi_traces[ind_start : ind_start + args.repetitions, :] = cellsb[
                :, start_timestep:max_timestep, 16
            ]
            cd8_traces[ind_start : ind_start + args.repetitions, :] = cellsb[
                :, start_timestep:max_timestep, 0
            ]
            ...
            ax.plot(
                t, np.transpose(cellsb[:, :, 16]), color="C0", linewidth=1, alpha=0.35
            )
            fig.savefig(
                os.path.join(
                    args.result, "epithelials_traces_%s%s.png" % (patient, gene)
                ),
                dpi=600,
                pad_inches=0.1,
                bbox_inches="tight",
            ...
            fig, ax = plt.subplots(figsize=(6, 4))
            ax.plot(
                t, np.transpose(cellsb[:, :, 0]), color="C0", linewidth=1, alpha=0.35
            )
            fig.savefig(
                os.path.join(args.result, "cd8_traces_%s%s.png" % (patient, gene)),
                dpi=600,
                pad_inches=0.1,
                bbox_inches="tight",
            )  # dpi=600,
            ...
            fig.savefig(
                os.path.join(args.result, "epithelials_%s%s.png" % (patient, gene)),
                dpi=600,
                pad_inches=0.1,
                bbox_inches="tight",
            )  # dpi=600,
            ...
            fig.savefig(
                os.path.join(args.result, "macrophages_%s%s.png" % (patient, gene)),
                dpi=600,
                pad_inches=0.1,
                bbox_inches="tight",
            )  # dpi=600,
            ...
            ax.plot(t, cells[0, 0, :])
            fig.savefig(
                os.path.join(args.result, "cd8s_%s%s.png" % (patient, gene)),
                dpi=600,
                pad_inches=0.1,
                bbox_inches="tight",
            )  # dpi=600,
    ...
    columns_traces = [
        "epi_%d" % index for index in range(max_timestep - start_timestep)
    ]
    columns_traces += [
        "cd8_%d" % index for index in range(max_timestep - start_timestep)
    ]
    rows_traces = [
        "%s%s_%s" % (patient, gene[:-1], i_rep)
        for patient in patients
        for gene in genes
        for i_rep in range(args.repetitions)
    ]
    ...
    for i, patient in enumerate(patients):
        results_patients[i, :] = np.mean(
            results[i * len(patients) : i * len(patients) + 2, :], axis=0
        )
    ...
    plt.savefig(
        os.path.join(args.result, "dendogram_patients.png"),
        dpi=600,
        pad_inches=0.1,
        bbox_inches="tight",
    )
    ...
    fig_sns.savefig(
        os.path.join(args.result, "clustermap_patients.png"),
        dpi=600,
        pad_inches=0.1,
        bbox_inches="tight",
    )
    ...
    ax.tick_params(axis="x", which="major", labelsize=5)
    plt.savefig(
        os.path.join(args.result, "dendogram_genes.png"),
        dpi=600,
        pad_inches=0.1,
        bbox_inches="tight",
    )
    ...
    fig_sns = sns_plot._figure
    fig_sns.savefig(
        os.path.join(args.result, "clustermap_genes.png"),
        dpi=600,
        pad_inches=0.1,
        bbox_inches="tight",
    )
    ...
    ax.tick_params(axis="x", which="major", labelsize=5)
    plt.savefig(
        os.path.join(args.result, "dendogram.png"),
        dpi=600,
        pad_inches=0.1,
        bbox_inches="tight",
    )
    ...
    fig_sns = sns_plot._figure
    fig_sns.savefig(
        os.path.join(args.result, "clustermap.png"),
        dpi=600,
        pad_inches=0.1,
        bbox_inches="tight",
    )
    ...
    ax.tick_params(axis="x", which="major", labelsize=5)
    plt.savefig(
        os.path.join(args.result, "dendogram_traces.png"),
        dpi=600,
        pad_inches=0.1,
        bbox_inches="tight",
    )
    ...
    fig_sns = sns_plot._figure
    fig_sns.savefig(
        os.path.join(args.result, "clustermap_traces.png"),
        dpi=600,
        pad_inches=0.1,
        bbox_inches="tight",
    )
