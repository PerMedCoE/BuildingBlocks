...
- [Building Block cll_run_boolean_model](#building-block-new_name)
...
```bash
pip install cll_run_boolean_model
```
...
- Command line interface:
    ...
    Once installed the `cll_run_boolean_model` package, it provides the `cll_run_boolean_model`
    command, that can be used from the command line. For example:

    ```text
    $ cll_run_boolean_model -h
    usage: cll_run_boolean_model [-h] --model MODEL --result RESULT [-c CONFIG] [-d] [-l {debug,info,warning,error,critical}] [--tmpdir TMPDIR] [--processes PROCESSES] [--gpus GPUS] [--memory MEMORY]
                [--mount_points MOUNT_POINTS]

    cll_run_boolean_model Building Block short description. Give more details about the Building Block.
    ...
    ```python
    from cll_run_boolean_model import invoke
...
- Extension for PyCOMPSs:
    ...
    ```python
    from cll_run_boolean_model import cll_run_boolean_model_extended

    cll_run_boolean_model_extended(*args, **kwargs)  # specific interface
    ```
...
```bash
pip uninstall cll_run_boolean_model
```
...
- Provide a Python script `main.py` with the following structure and adapt to your needs (check all `ToDo` marked lines):
  ...
  from cll_run_boolean_model.definitions import cll_run_boolean_model_ASSETS_PATH  # binary could be in this folder
  from cll_run_boolean_model.definitions import cll_run_boolean_model_CONTAINER
  ...
  @binary(binary="cp")                                        # ToDo: Define the binary to be used (can be within cll_run_boolean_model_ASSETS_PATH (e.g. my_binary.sh)).
  ...
  - A BB can be a single executable, but it can be a more complex code if the `cll_run_boolean_model_extended` function is implemented and used with PyCOMPSs.
  ...
  parses the Yaml config file and invokes the `cll_run_boolean_model` function
  with the appropriate parameters.

  - The BB `binary` must be defined with the `@task`, `@binary` and `@container` decorators (`cll_run_boolean_model_task`). This function needs to declare the binary flags, and it is invoked from the `cll_run_boolean_model` function.
