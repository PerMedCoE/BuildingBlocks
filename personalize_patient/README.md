...
This package provides the Personalize Patient **Building Blocks (BB)**.

## Table of Contents

- [HPC/Exascale Centre of Excellence in Personalised Medicine](#hpcexascale-centre-of-excellence-in-personalised-medicine)
  - [Personalize Patient Pilot Building Block](#personalize-patient-building-block)
...
- From Pypi:
  ...
  ```bash
  pip install personalize_patient_BB
  ```

  or more specifically:

  ```bash
  python3 -m pip install personalize_patient_BB
  ```
...
The `personalize_patient_BB` package provides a clear interface that allows it to be
...
```bash
pip uninstall personalize_patient_BB
```
